...
var math_One = (Math.floor( Math.random() * 15 ) + 1);
var math_Two = (Math.floor( Math.random() * 15 ) + 1);
var math_Three = (math_One * math_Two);
var math_MinusOne = (Math.floor( Math.random() * 15 ) + 1);
var math_MinusTwo = (Math.floor( Math.random() * 15 ) + 1);
var WronganswerOne = (math_Three -  math_MinusOne);
var WronganswerTwo = (math_Three -  math_MinusTwo);
...
var Enemy = SheetSprite.extend({
  init: function(clipRects, x, y, value) {
...
function setupMathProblem() {
  math_One = (Math.floor( Math.random() * 15 ) + 1);
  math_Two = (Math.floor( Math.random() * 15 ) + 1);
  math_Three = (math_One * math_Two);
  math_MinusOne = (Math.floor( Math.random() * 15 ) + 1);
  math_MinusTwo = (Math.floor( Math.random() * 15 ) + 1);
  WronganswerOne = (math_Three -  math_MinusOne);
  WronganswerTwo = (math_Three -  math_MinusTwo);
}

function setupAlienFormation() {
  alienCount = 0;
  var values = [WronganswerOne, WronganswerTwo, math_Three];
  shuffleArray(values);
  for (var i = 0, len = 1 * 3; i < len; i++) {
    var gridX = (i % 11);
    ...
    aliens.push(new Enemy(clipRects, (CANVAS_WIDTH/ 5- ALIEN_SQUAD_WIDTH/2) + ALIEN_X_MARGIN/2 + gridX * ALIEN_X_MARGIN, CANVAS_HEIGHT/3.25 - gridY * 40, values[i]));
    alienCount++;
  }
}
...
function drawBottomHud() {
  ...
  fillCenteredText(math_One + ' x ' + math_Two + ' =' + "?" , CANVAS_WIDTH/2, 20);
...
function drawStartScreen() {
  fillCenteredText("Math Invaders", CANVAS_WIDTH/2, CANVAS_HEIGHT/2.75, '#FFFFFF', 36);
